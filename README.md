...
- `/info` 查询过去 1 小时流量。
- `/report` 查询过去 6 小时流量。
- 单 IP 在滚动 5 分钟内达到 7 GiB 时报警，同一 IP 30 分钟内只提醒一次。
...
| 参数 | 默认 | 说明 |
| --- | --- | --- |
| `-check` | 关 | 仅检查 nftables 读取与本地存储，不启动机器人。 |
| `-data` | `data` | 数据目录（state.db 与按日 traffic-\*.db 的存放位置）。 |
| `-env` | `.env` | `.env` 文件路径；环境变量 `APPID` / `SECRET` 优先于文件中对应字段。 |
